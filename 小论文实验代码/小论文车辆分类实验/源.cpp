...
void opencvLbp(Mat& src)
{
	...
	radius = 1;
	neighbors = 8;
	Mat dst = Mat(srcImage.rows - 2 * radius, srcImage.cols - 2 * radius, CV_8UC1, Scalar(0));
	double t = (double)cv::getTickCount();
	elbp(srcImage, dst, radius, neighbors);
	t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
	cout << "opencv lbp cost time: " << t << endl;
	namedWindow("opencv lbp", WINDOW_AUTOSIZE);
	imshow("opencv lbp", dst);
}